...
export default {
  async execute(params: unknown, ctx: ToolContext): Promise<ToolResult> {
    ...
    for (const rawPath of inputPaths) {
      const result = await this.deleteSingle(
        rawPath,
        ctx,
  ...
  /**
   * 删除单个路径。此方法绑定到 default export 对象上，便于测试 mock。
   * 注意：此处用 async function 形式，this 指向 default export 对象。
   */
  async deleteSingle(
    rawPath: string,
    ctx: ToolContext,
    ...
    if (!absPath) {
      return { absPath: rawPath, success: false, message: `path escapes working directory` };
    }
